...
- **L'amplification a un seuil net.** Quand le gain algorithmique dépasse le taux d'oubli
  naturel d'un contenu ($\gamma\alpha > \lambda$), l'amortissement effectif devient
  négatif : le système accumule l'énergie au lieu de la dissiper. C'est un effet Larsen
  informationnel, et c'est une inégalité **auditable**.
...
L'**[audit critique](docs/limites.md)** recense quatorze corrections apportées au
raisonnement d'origine — dont **cinq formules invalides** — et énumère les limites qui
subsistent, y compris celles qui touchent à l'usage réglementaire de l'index : il est
manipulable, sa discrétisation en points de vue est un choix politique, et un seuil
imposé sur sa valeur est une intervention sur le débat public, non une simple mesure
technique.

Rien ici ne démontre que les opinions humaines *obéissent* à une mécanique statistique.
Le travail établit qu'un formalisme emprunté à la physique **reproduit** certains
comportements observés et en tire des quantités mesurables. Les paramètres n'ont
**aucune procédure de calibration empirique** : c'est la faiblesse principale, et la
[feuille de route](docs/feuille-de-route.md) dit comment y remédier.
...
docker compose run --rm test          # 203 tests, dont les exemples de docstrings
docker compose run --rm lint          # ruff
docker compose run --rm notebooks     # régénère les 8 figures de la note
...
tests/              203 tests — validation physique et numérique
notebooks/          01 à 08, un par bloc théorique, exécutables
...
- [`docs/limites.md`](docs/limites.md) — **audit critique** : les quatorze corrections et
